/**
 * Pipeline Flow & Agentic Node Workflow Renderer — n8n style node pipeline & execution history log
 */
const PipelinePage = {
  async render(container) {
    container.innerHTML = `
      <!-- n8n Style Interactive Agentic Node Workflow Header -->
      ...
      <!-- n8n Visual Workflow Diagram Card -->
      <div class="glass-card" style="padding: 30px; margin-bottom: 28px; background: var(--bg-card); overflow-x: auto;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px; min-width: 850px;">
          ...
          <!-- Node 3: Gemini 2.5 Flash Rewriter -->
          <div style="flex: 1; background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 8px; padding: 18px; text-align: center; position: relative;">
            <div style="width: 10px; height: 10px; border-radius: 50%; background: #2e7d32; position: absolute; top: 12px; right: 12px;"></div>
          ...
          <!-- Node 4: Gemini 2.0 Image Studio -->
          <div style="flex: 1; background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 8px; padding: 18px; text-align: center; position: relative;">
            <div style="width: 10px; height: 10px; border-radius: 50%; background: #2e7d32; position: absolute; top: 12px; right: 12px;"></div>
            ...
            <p style="font-size: 0.75rem; color: var(--text-muted);">Gemini 2.0 Flash Images</p>
      ...
      <!-- Execution Runs History Table (Scalability & Saved Logs Archive) -->
  ...
  async loadPipelineData() {
    try {
      ...
      if (!runs || runs.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8">No execution history recorded yet. Click "Run Agentic Pipeline" above!</td></tr>';
        return;
      }
